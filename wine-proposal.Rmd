...
**Points**: the number of points the wine received in its review, on a scale of 1-100. However, only wines with >= 80 points were published, so we will transform this. We plan to check out logit, probit, BoxCox and log transforms. This will be our response variable.  
...
We also plan to create a categorical continentTopFive, in which wines are separated into continent of origin, with the exception of partitioning off the top 5 wine producers in Europe. There are given here: http://www.businessrevieweurope.eu/top10/250/Top-10:-Wine-Producing-Countries-in-Europe  
Partitioning wine by continent separates regressions nicely, with changes of slope and intercept for various continents.  Further, when we isolated the top 5 producers of Europe (France, Germany, Italy, Portugal, Spain) we see that "OtherEurope" is nicely separated with a unique slope (larger).  These top 5 dominate the overall regression, and so isolating allows more predictive power for the "edge cases" of other European countries.
