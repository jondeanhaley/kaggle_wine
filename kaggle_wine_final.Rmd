...
- The optimal model after backward BIC selection was:  $log(points - 79.999) \sim log(price) + continentTopFive + topic1 + I(log(price)^2) + I(log(price)^3) + log(price):continentTopFive + continentTopFive:topic1$
...
select_formula1 = log(points - 79.999) ~ log(price) + continentTopFive + 
    topic1 + I(log(price)^2) + I(log(price)^3) + log(price):continentTopFive + 
    continentTopFive:topic1
...
- The optimal model after backward BIC selection was:  $points \sim log(price) + continentTopFive + topic1 + I(log(price)^2) + I(log(price)^3) + log(price):continentTopFive + log(price):topic1 + continentTopFive:topic1$
...
- The optimal model after backward BIC selection was:  $bc\_points \sim log(price) + continentTopFive + topic1 + I(log(price)^2) + I(log(price)^3) + log(price):continentTopFive + log(price):topic1 + continentTopFive:topic1$
...
- The model started with the base of: $points \sim log(price)*continentTopFive*sentiment*topic1 + I(log(price)^2) + I(log(price)^3) + I(log(price)^4) + I(log(price)^5) + I(log(price)^6) + I(log(price)^7)$
- The optimal model after backward BIC selection was:  $points \sim log(price) + continentTopFive + topic1 + I(log(price)^2) + I(log(price)^3) + I(log(price)^4) + I(log(price)^5) + I(log(price)^6) + I(log(price)^7) + log(price):continentTopFive + log(price):topic1 + continentTopFive:topic1$
...
#update
select_formula5 = points ~ log(price) + continentTopFive + topic1 + 
    I(log(price)^2) + I(log(price)^3) + I(log(price)^4) + I(log(price)^5) + 
    I(log(price)^6) + I(log(price)^7) + log(price):continentTopFive + 
    log(price):topic1 + continentTopFive:topic1
...
The goal of this project, as well as the kaggle competition (from where we got the data) was to see if the `wine points` could be predicted accurately as against trying to explain what determins the `wine points`. Although we used linear regression models in this project, one could argue that use of tree enseble based models such as GBM or random forest may be better at this task.
...
Due to time constraints we couldn't really work on creating more topics and evaluating their impact on the models. Trying to create LDA with 3 topics didn't show as much improvement in the model performance. One could consider spending time to create more number of topics in the future.
